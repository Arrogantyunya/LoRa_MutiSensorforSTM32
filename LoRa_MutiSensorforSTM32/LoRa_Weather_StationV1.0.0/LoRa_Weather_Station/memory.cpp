...
{
    EP_Write_Enable();
    EEPROM1024.write(LORA_PARA_CONFIG_FLAG_ADDR, 0x55);
    EP_Write_Disable();

    if (EEPROM1024.read(LORA_PARA_CONFIG_FLAG_ADDR) == 0x55)
        return true;
    else
        return false;
}
...
{
    EP_Write_Enable();
    EEPROM1024.write(LORA_PARA_CONFIG_FLAG_ADDR, 0x00);
    EP_Write_Disable();   

    if (EEPROM1024.read(LORA_PARA_CONFIG_FLAG_ADDR) == 0x00)
        return true;
    else
        return false;
}